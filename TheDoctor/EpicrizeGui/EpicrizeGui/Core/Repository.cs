	...
	{		
		...
		private static TEntity ReadEntity<TEntity>(SqlDataReader reader) where TEntity : new()
		{
			...
			foreach(PropertyInfo property in type.GetProperties())
			{
				...
				try
				{
					index = reader.GetOrdinal(property.Name);				
				}
				...
				if(index >= 0)
				{
					...
					{
						property.SetValue(entity, reader.GetInt32(index), null);
					}
					...
					{
						property.SetValue(entity, reader.GetFloat(index), null);
					}
                    else if(valueType.Equals(typeof(double)))
					{
						//property.SetValue(entity, reader.GetDouble(index), null);
						//object value = reader.GetDouble(index);
						property.SetValue(entity, (float)reader.GetDouble(index), null);
					}              
